    ...
    <div className="min-h-screen pt-20 pb-20 relative">
      ...
      <section className="relative py-16 overflow-hidden z-10">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          <h1 className="font-display text-5xl md:text-6xl font-black text-white mb-6 uppercase tracking-wider">
            Jadwal <span className={`text-${filterColor}`}>{filterCabang ? filterCabang : 'Latihan'}</span>
          </h1>
          
          <div className="flex justify-center gap-4 mt-8">
            <Link href="/jadwal" className={`px-6 py-2 rounded-xl font-bold uppercase tracking-widest border transition-all ${!filterCabang ? 'bg-white/10 border-white/20 text-white' : 'border-transparent text-neutral-light/50 hover:text-white'}`}>
              Semua
            </Link>
            <Link href="/jadwal?cabang=Basket" className={`px-6 py-2 rounded-xl font-bold uppercase tracking-widest border transition-all ${filterCabang === 'Basket' ? 'bg-basket/20 border-basket/40 text-basket' : 'border-transparent text-neutral-light/50 hover:text-white'}`}>
              🏀 Basket
            </Link>
            <Link href="/jadwal?cabang=Renang" className={`px-6 py-2 rounded-xl font-bold uppercase tracking-widest border transition-all ${filterCabang === 'Renang' ? 'bg-renang/20 border-renang/40 text-renang' : 'border-transparent text-neutral-light/50 hover:text-white'}`}>
              🏊 Renang
            </Link>
          </div>
        </div>
      ...
      <section className="py-12 z-10 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center gap-3 mb-10 border-b border-white/10 pb-4">
            <h2 className="font-display text-3xl font-black text-white uppercase tracking-wider">Latihan Mingguan</h2>
          </div>
                ...
                <div key={j.id} className={`matchday-card ${j.cabang_olahraga === 'Basket' ? 'border-basket' : 'border-renang'}`}>
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      ...
                      <h3 className="font-display text-2xl font-bold text-white uppercase">{j.kategori}</h3>
                    </div>
                  ...
                  <div className="space-y-3 mt-6 border-t border-white/5 pt-4">
                    <div className="flex items-center gap-3 text-neutral-light/80">
                      ...
                      <span className="font-mono text-lg font-bold">{j.jam_mulai} <span className="text-white/30 text-sm font-sans mx-1">s/d</span> {j.jam_selesai}</span>
                    </div>
                    <div className="flex items-center gap-3 text-neutral-light/80">
                      <div className="w-8 h-8 rounded-full bg-white/5 flex items-center justify-center shrink-0">
      ...
      <section className="py-12 z-10 relative">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-3 mb-10 border-b border-white/10 pb-4">
            <h2 className="font-display text-3xl font-black text-white uppercase tracking-wider">Pertandingan & Event</h2>
          </div>

          {jadwalTanding.length === 0 ? (
            <div className="glass-card border border-white/5 rounded-2xl p-12 text-center max-w-3xl mx-auto">
                ...
                <div key={j.id} className="scoreboard-card group">
                  ...
                  <div className="flex flex-col sm:flex-row gap-6 relative z-10">
                    <div className="flex-1">
                      ...
                      <h3 className="font-display text-3xl font-black text-white uppercase tracking-wide mb-1">{j.keterangan || 'Matchday'}</h3>
                    ...
                    <div className="sm:w-48 bg-arena-800 rounded-xl border border-white/5 p-4 flex flex-col justify-center items-center text-center">
                      <div className="text-neutral-light/40 text-xs font-bold uppercase tracking-widest mb-2">Kick Off</div>
                      <div className="font-mono text-3xl font-bold text-white mb-1">{j.jam_mulai}</div>
