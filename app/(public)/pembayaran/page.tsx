      ...
      <section className="pb-24">
        <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8">
              ...
              return (
                <button key={c} onClick={() => setCabangFilter(c)}
                  className={`flex items-center gap-2 px-6 py-3 rounded-xl font-semibold transition-all ${
                    cabangFilter === c ? `${cfg.bg} border ${cfg.color}` : 'text-white/50 hover:text-white hover:bg-white/10 border border-transparent'
                  }`}>
                  {cfg.emoji} {c}
                </button>
              );
          ...
          <div className="glass-card border rounded-2xl overflow-hidden">
            <div className={`${cabangConfig[cabangFilter].bg} border-b border-white/10 px-6 py-4 flex items-center gap-3`}>
              <span className="text-2xl">{cabangConfig[cabangFilter].emoji}</span>
              <h2 className="font-display text-xl font-bold text-white">SPP Cabang {cabangFilter}</h2>
            </div>
          ...
          <div className="glass-card border border-amber-500/20 bg-amber-500/5 rounded-2xl p-6 flex items-start gap-4">
            ...
            <div>
              <h3 className="font-semibold text-white mb-1">Jadwal Jatuh Tempo</h3>
              <p className="text-white/60">{jatuhTempo}</p>
          ...
          {/* Metode Pembayaran */}
          <div>
            <h2 className="font-display text-2xl font-bold text-white mb-6">Metode Pembayaran</h2>
            ...
            <div className="glass-card border border-emerald-500/30 bg-emerald-500/5 rounded-2xl p-6 mb-4">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  <h3 className="font-semibold text-white">Bayar Online — Otomatis Tercatat ✨</h3>
                  <p className="text-white/50 text-sm">QRIS · Virtual Account BCA/BNI/BRI/Mandiri</p>
                </div>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              ...
              {branding?.rek_bank_nomor && (
                <div className="glass-card-hover border rounded-2xl p-6">
                  ...
                  <h3 className="font-semibold text-white mb-3">Transfer Bank Manual</h3>
                  <div className="space-y-2 text-sm">
                    <div className="flex justify-between"><span className="text-white/50">Bank</span><span className="text-white font-medium">{branding.rek_bank_nama}</span></div>
                    <div className="flex justify-between"><span className="text-white/50">Rekening</span><span className="text-white font-mono font-medium">{branding.rek_bank_nomor}</span></div>
                    <div className="flex justify-between"><span className="text-white/50">Atas Nama</span><span className="text-white font-medium">{branding.rek_bank_atas_nama}</span></div>
                  </div>
              ...
              <div className="glass-card-hover border rounded-2xl p-6">
                ...
                <h3 className="font-semibold text-white mb-3">Pembayaran Tunai</h3>
          ...
          <div className="glass-card border rounded-2xl p-6">
            <h2 className="font-display text-xl font-bold text-white mb-4">Cek Status Pembayaran Mandiri</h2>
            ...
            {checkData !== null && (
              <div className="space-y-2">
                {checkData.length === 0 ? (
                  <p className="text-white/40 text-sm text-center py-4">Tidak ada data pembayaran untuk ID tersebut</p>
                ) : (
                  checkData.map((d, i) => (
                    <div key={i} className="flex items-center justify-between p-3 bg-white/5 rounded-xl text-sm">
                      <span className="text-white font-medium">{d.nama_anggota} — {d.bulan}/{d.tahun}</span>
